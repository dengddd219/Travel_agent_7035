...
### [3.9final for CV] - 2026-03-09

#### 代码核查与简历交付收口
- **核查范围**：对当前代码进行全量可执行核查（语法编译 + 测试 + IDE lints）。
- **核查结果**：
  - `python -m compileall src`：通过（语法层面无阻塞错误）。
  - IDE `ReadLints`：未发现新增 lints 问题。
  - `pytest -q`（全量）：当前仓库存在历史失败/错误用例（主要在 integration/e2e 相关），本次未做额外修复，保留现状以避免引入回归。
- **本轮功能收口（已同步）**：
  - Evaluation Panel：历史结果抽屉化、历史重算、细节与对比图展示、开发备忘区。
  - LLM Arena + Evaluation Panel：评分口径统一为传统 Min-Max 综合评分（质量/延迟/成本），并移除路由准确率列与路由计分。
  - Chat 侧产品规范：用户侧仅保留 `快速 / 思考 / Pro` 三档模式（不暴露底层模型细节）。
- **交付标记**：`[x] FINAL FOR CV`

---

### [update[3.9] Chat英文文案自适应] - 2026-03-09

#### Chat 检索状态与引用标题中英自适应
- **状态提示文案切换[update[3.9]]**：当用户输入英文 query，或显式要求英文回答（如 `answer in English` / `请用英文回答`）时，Chat 加载态文案切换为英文：`Retrieving knowledge base and generating answer...`。
- **引用标题文案切换[update[3.9]]**：同一语言判定条件下，引用折叠标题由 `📚 引用来源（N）` 切换为 `📚 Sources (N)`。
- **页码文案切换[update[3.9]]**：引用来源页码在英文模式下显示为 `(page X)`，中文模式保持 `(第 X 页)`。
- **会话一致性[update[3.9]]**：assistant 消息持久化记录 UI 语言，历史会话回放时保持与生成时一致的引用文案语言。
- **实现位置**：`src/observability/dashboard/pages/chat_interface.py`

---

### [update3.9聊天模式选择] - 2026-03-09

#### Chat 模式化选择（与 LLM Arena 联动）
- **用户侧仅暴露 3 档模式**：在 Chat 界面仅提供 `快速`、`思考`、`Pro` 三种模式，不再展示具体模型名称。
- **模式语义**：
  - `快速`：偏速度与成本友好，适合高频问答。
  - `思考`：偏准确率与稳定性，适合复杂问题。
  - `Pro`：偏综合体验，作为高质量默认档位。
- **与 LLM Arena 联动**：三档模式的后端路由可参考 LLM Arena 最新基准结果动态调整，但该细节不暴露给终端用户。
- **产品约束**：用户只做“模式选择”，不承担模型配置复杂度；模型映射属于系统内部实现策略。
...
#### Chat界面增强
- **模式选择器功能[update3.9]**：在Chat界面侧边栏添加模式选择器，仅提供 `快速` / `思考` / `Pro` 三种选项，用于控制速度、成本、准确率的权衡。
  - 仅展示模式，不展示具体模型名称。
  - 模式到模型/路由策略的映射由系统内部管理，可依据 LLM Arena 结果调整。
  - 使用 `ModelManager`（或等价路由层）统一管理模式映射与切换。
  - 实现位置：`src/observability/dashboard/pages/chat_interface.py`
...
- **中英文案自适应[update[3.9]]**：当 query 为英文或显式要求英文回答时，检索状态文案与引用来源标题自动切换英文（`Retrieving knowledge base and generating answer...`、`📚 Sources (N)`），页码显示为 `(page X)`。
...
- **模式选择器功能[update3.9]**：在侧边栏添加 `快速` / `思考` / `Pro` 三档模式选择，用于控制成本、速度与准确率权衡；不向用户展示底层模型名称。
...
| G8 | Chat界面模型/模式选择与运行时间展示[update3.8+3.9] | [x] | 2026-03-09 | Chat 侧边栏提供三档模式（快速/思考/Pro）供用户选择，不展示底层模型；添加总运行时间展示；在LLM Arena中展示生成时长和测评时长（单次和批量评估） |
...
- **实现要点**：
  ...
  - 中英文案自适应[update[3.9]]：英文 query/英文回答场景下，状态提示与引用标题自动切英文，页码显示切换为 `(page X)`
  - 会话历史持久化：保存到 `data/chat/conversations.json`，支持历史对话恢复
- **验收标准**：可在 Dashboard 中发起对话，查看引用来源，重启后恢复历史对话。
- **测试方法**：手动验证 Chat 页面功能。

### G8：Chat界面模式选择器与运行时间展示[update3.8+3.9]
- **目标**：在Chat界面添加三档模式选择器（快速/思考/Pro），让用户按成本-速度-准确率偏好选择；并添加运行时间展示功能。
