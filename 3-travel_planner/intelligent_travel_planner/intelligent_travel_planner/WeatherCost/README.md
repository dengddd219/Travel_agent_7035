...
The main file is [weather_cost_api.py](./weather_cost_api.py).
...
- [weather_cost_api.py](./weather_cost_api.py): main implementation
- [README.md](./README.md): usage notes for the flattened WeatherCost folder

All commands in this README assume your terminal is already inside the project folder that contains `weather_cost_api.py`.
...
cd path\to\WeatherCost
python -m pip install requests playwright
python -m playwright install chromium
```

Replace `path\to\WeatherCost` with the local path to this project on your own machine.

## Configuration

Create a local `.env` file manually with the keys below.
...
conda activate groupc-playwright
cd path\to\WeatherCost
python -c "from weather_cost_api import prepare_ctrip_hotel_login_api; print(prepare_ctrip_hotel_login_api())"
```
...
conda activate groupc-playwright
cd path\to\WeatherCost
python -c "from weather_cost_api import search_hotels_api; import json; hotels = search_hotels_api('上海','2026-05-01','2026-05-03'); print('count=', len(hotels)); print('sources=', sorted(set(h.get('source') for h in hotels if h.get('source')))); print(json.dumps(hotels[:3], ensure_ascii=False, indent=2))"
```
