    ...
    if requests:
        for request_id, request in requests.items():
            if request.type == "function_approval_request":
                ...
                print(f"Human approval for: {request.function_call.name}")  # type: ignore
                human_input=input("Approve this tool call? [y/N]: ")
                human_decision=False
                if human_input == "y" or human_input == "Y":
                    human_decision = True
                # Create approval response
                responses[request_id] = request.to_function_approval_response(approved=human_decision)
